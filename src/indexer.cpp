...
bool Indexer::saveIndex(const string &filename) const {
    ...
    ofs.write((char*)&shards, sizeof(shards));
    int N = (int)docs.size();
    ofs.write((char*)&N, sizeof(N));

    for (const auto &d : docs) {
        uint64_t plen = (uint64_t)d.path.size();
        ofs.write((char*)&plen, sizeof(plen));
        ofs.write(d.path.data(), plen);
        ofs.write((char*)&d.size_bytes, sizeof(d.size_bytes));
        ofs.write((char*)&d.mtime, sizeof(d.mtime));
    }

    // persist token counts so avgDocLen is correct on load
    for (int i = 0; i < N; ++i) {
        int tc = docTokenCount[i];
        ofs.write((char*)&tc, sizeof(tc));
    }

    uint64_t vocabCount = df.size();
    ofs.write((char*)&vocabCount, sizeof(vocabCount));
    for (auto &p : df) {
        const string &term = p.first;
        uint64_t tlen = (uint64_t)term.size();
        ofs.write((char*)&tlen, sizeof(tlen));
    ...
    for (int sh = 0; sh < shardCount; ++sh) {
        ...
        ofs.write((char*)&termsCount, sizeof(termsCount));
...
bool Indexer::loadIndex(const string &filename) {
    ...
    ifs.read(magic,4);
    if (string(magic) != "IDX1") return false;
...
string Indexer::makeSnippet(int docID, const vector<string> &qterms, size_t window) const {
    const string &content = docs.at(docID).content;
    ...
    for (auto &q : qterms) {
        ...
        if (pos != string::npos) {
            if (bestPos == string::npos || pos < bestPos) { bestPos = pos; bestTerm = qlower; }
        }
    }
    if (bestPos == string::npos) {
        size_t n = min((size_t)150, content.size());
        return content.substr(0,n);
    }
    size_t start = (bestPos > window ? bestPos - window : 0);
    size_t end = min(content.size(), bestPos + bestTerm.size() + window);
...
void Indexer::preloadHotDocs(int k) {
    // Rank docs by total df-weighted term count (proxy for "hotness")
    vector<pair<int,int>> ranked; // {docID, tokenCount}
    ...
    for (int i = 0; i < limit; ++i) {
        ...
        if (docs[id].content.empty() && !docs[id].path.empty())
            docs[id].content = loadFileContent(docs[id].path);
    }
    Logger::instance().log(LINFO, "Preloaded " + to_string(limit) + " hot docs into memory");
}
