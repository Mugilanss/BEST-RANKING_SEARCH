...
Indexer::Indexer() {
    ...
    shardMutexes.clear();
    for (int i = 0; i < shardCount; ++i) shardMutexes.emplace_back(make_unique<mutex>());
    trie = nullptr;
    bk   = nullptr;
}
...
Indexer::~Indexer() {
    ...
    if (bk) { delete bk; bk = nullptr; }
}

// ---------------------- Config setters/getters ----------------------
void Indexer::setExtensionFilter(const std::string &ext) { extFilter = ext; }
void Indexer::setMaxThreads(int n)   { if (n>0) maxThreads = n; }
...
{
    string content;
    if (tokenizer) content = loadFileContent(path);
    else content = loadFileContent(path); // still load

    vector<string> tokens;
    if (tokenizer) tokens = tokenizer->tokenize(content);
    ...
    try { docs[docID].mtime = ft_to_time_t(fs::last_write_time(path)); } catch(...) { docs[docID].mtime = 0; }
...
void Indexer::buildFromFileList(const vector<string> &files) {
    ...
    for (size_t i = 0; i < Nfiles; ++i) {
        ...
        for (auto &entry : local) {
            ...
            { lock_guard<mutex> lk(*shardMutexes[sh]); postingsSharded[sh][tp].push_back({docID, entry.second}); }
            seen.insert(tp);
        }
        { lock_guard<mutex> lk(metaMu); for (auto tp : seen) df[*tp] += 1; }
    }
    int N = (int)docs.size();
    { lock_guard<mutex> lk(metaMu); idf.clear(); for (auto &p : df) idf[p.first] = log((double)(N+1)/(p.second+1))+1.0; }
    for (int i = 0; i < N; ++i) {
        double sum = 0.0;
        for (auto &pr : docTf[i]) { auto it=idf.find(*pr.first); double iv=(it==idf.end())?0.0:it->second; sum+=pr.second*iv*pr.second*iv; }
        docs[i].norm = sqrt(sum);
    }
    double tot = 0; for (int c : docTokenCount) tot += c;
...
void Indexer::buildFromFolderParallel(const string &folder, bool recursive) {
    ...
    for (size_t i = 0; i < Nfiles; ++i) {
        ...
        }
        {
            ...
            for (const string *tp : seen) df[*tp] += 1;
        }
    }
...
bool Indexer::loadIndex(const string &filename) {
    ...
    // Handle old format (IDX1 without compression) for backward compatibility
        ...
        int shards = 0; ifs.read((char*)&shards, sizeof(shards));
        shardCount = shards;
        ...
        int N = 0; ifs.read((char*)&N, sizeof(N));
        docs.clear(); docs.resize(N);
        docTf.clear(); docTf.resize(N);
        docTokenCount.assign(N, 0);

        for (int i = 0; i < N; ++i) {
            uint64_t plen; ifs.read((char*)&plen, sizeof(plen));
            string path; path.resize(plen);
        ...
        uint64_t vocabCount = 0; ifs.read((char*)&vocabCount, sizeof(vocabCount));
        df.clear(); vocabMap.clear();
        for (uint64_t i = 0; i < vocabCount; ++i) {
            uint64_t tlen; ifs.read((char*)&tlen, sizeof(tlen));
            string term; term.resize(tlen); ifs.read(&term[0], tlen);
            int dfv; ifs.read((char*)&dfv, sizeof(dfv));
            df[term] = dfv;
        ...
        for (int sh = 0; sh < shardCount; ++sh) {
            uint64_t termsCount = 0; ifs.read((char*)&termsCount, sizeof(termsCount));
            for (uint64_t ti = 0; ti < termsCount; ++ti) {
                uint64_t tlen; ifs.read((char*)&tlen, sizeof(tlen));
                string term; term.resize(tlen); ifs.read(&term[0], tlen);
                const string *termPtr = interner.intern(term);
                vocabMap[term] = termPtr;
                uint64_t plistSize = 0; ifs.read((char*)&plistSize, sizeof(plistSize));
                vector<Posting> plist; plist.reserve((size_t)plistSize);
                int prevDoc = 0;
                for (uint64_t pi = 0; pi < plistSize; ++pi) {
                    ...
                    vector<int> positions; positions.reserve((size_t)posCount);
                    int prevPos = 0;
...
bool Indexer::loadIndexFromStream(istream &stream) {
    int shards = 0; stream.read((char*)&shards, sizeof(shards));
    shardCount = shards;
    ...
    int N = 0; stream.read((char*)&N, sizeof(N));
    docs.clear(); docs.resize(N);
    docTf.clear(); docTf.resize(N);
    docTokenCount.assign(N, 0);

    for (int i = 0; i < N; ++i) {
        uint64_t plen; stream.read((char*)&plen, sizeof(plen));
        string path; path.resize(plen);
    ...
    uint64_t vocabCount = 0; stream.read((char*)&vocabCount, sizeof(vocabCount));
    df.clear(); vocabMap.clear();
    for (uint64_t i = 0; i < vocabCount; ++i) {
        uint64_t tlen; stream.read((char*)&tlen, sizeof(tlen));
        string term; term.resize(tlen); stream.read(&term[0], tlen);
        int dfv; stream.read((char*)&dfv, sizeof(dfv));
        df[term] = dfv;
        const string* tp = interner.intern(term);
        vocabMap[term] = tp;
    }
    ...
    for (int sh = 0; sh < shardCount; ++sh) {
        uint64_t termsCount = 0; stream.read((char*)&termsCount, sizeof(termsCount));
        for (uint64_t ti = 0; ti < termsCount; ++ti) {
            uint64_t tlen; stream.read((char*)&tlen, sizeof(tlen));
            string term; term.resize(tlen); stream.read(&term[0], tlen);
            const string *termPtr = interner.intern(term);
            vocabMap[term] = termPtr;
            uint64_t plistSize = 0; stream.read((char*)&plistSize, sizeof(plistSize));
            vector<Posting> plist; plist.reserve((size_t)plistSize);
            int prevDoc = 0;
            for (uint64_t pi = 0; pi < plistSize; ++pi) {
...
bool Indexer::finalizeLegacyLoad() {
    ...
    idf.clear();
    for (auto &p : df) idf[p.first] = log((double)(Nc + 1) / (p.second + 1)) + 1.0;
    ...
    // Remove any docs whose files no longer exist on disk
    for (int i = 0; i < (int)docs.size(); ++i) {
        if (!docs[i].path.empty() && !fs::exists(docs[i].path)) {
            Logger::instance().log(LINFO, "loadIndex: removing deleted file from index: " + docs[i].path);
            removeDocument(i);
        }
    }
...
void Indexer::incrementalUpdateWithWAL(const string &folder, WAL &wal, bool recursive) {
    ...
    for (auto &d : docs) known.insert(d.path);
    vector<string> added;
    if (recursive) {
        for (auto &p : fs::recursive_directory_iterator(folder)) {
            if (!fs::is_regular_file(p.path())) continue;
            string path = p.path().string();
            if (!known.count(path)) added.push_back(path);
        }
    } else {
        for (auto &p : fs::directory_iterator(folder)) {
            if (!fs::is_regular_file(p.path())) continue;
    ...
    for (auto &p : added) wal.appendAdd(p);
    ...
    int N = (int)docs.size();
    {
        lock_guard<mutex> lk(metaMu);
        idf.clear();
        for (auto &e : df) idf[e.first] = log((double)(N + 1) / (e.second + 1)) + 1.0;
    }
    ...
    double tot2 = 0.0;
    for (int c : docTokenCount) tot2 += c;
...
Indexer::SyncResult Indexer::syncFolder(const string &folder, bool recursive) {
    ...
    auto scanDir = [&](auto &iter) {
        for (auto &p : iter) {
            ...
            if (ext == ".txt" || ext == ".md" || ext == ".markdown" || ext == ".pdf") {
                time_t mt = 0;
                try { mt = ft_to_time_t(fs::last_write_time(p.path())); } catch(...) {}
                onDisk[p.path().string()] = mt;
            }
        }
    };
    try {
        if (recursive) { auto it = fs::recursive_directory_iterator(folder); scanDir(it); }
        else           { auto it = fs::directory_iterator(folder);            scanDir(it); }
    } catch (...) {
    ...
    for (int i = 0; i < (int)docs.size(); ++i)
        if (!docs[i].path.empty()) indexed[docs[i].path] = i;
    ...
    for (auto &[path, mt] : onDisk)
        if (!indexed.count(path)) toAdd.push_back(path);
    ...
    if (!toAdd.empty()) {
        ...
        for (int i = 0; i < newCount; ++i) {
            ...
            for (auto &kv : local) {
                ...
                vocabMap[*tp] = tp;
                size_t sh = shardFor(tp, shardCount);
                { lock_guard<mutex> lk(*shardMutexes[sh]); postingsSharded[sh][tp].push_back({docID, positions}); }
                seen.insert(tp);
            }
            { lock_guard<mutex> lk(metaMu); for (auto tp : seen) df[*tp] += 1; }
        }
    ...
    int N = (int)docs.size();
    { lock_guard<mutex> lk(metaMu); idf.clear(); for (auto &e : df) idf[e.first] = log((double)(N+1)/(e.second+1)) + 1.0; }
    for (int i = 0; i < N; ++i) {
        double sum = 0.0;
        for (auto &pr : docTf[i]) {
            double tfw = pr.second;
    ...
    Logger::instance().log(LINFO, "syncFolder done: +" + to_string(result.added)
        + " -" + to_string(result.removed) + " ~" + to_string(result.modified));
    return result;
}
...
vector<ScoredDoc> Indexer::searchTFIDF(const vector<string> &qterms, int topK) const {
    ...
    for (auto &term : qterms) {
        ...
        for (int id : ids) {
            ...
            if (vit == vocabMap.end()) continue;
            const string *tp = vit->second;
            auto tfit = docTf[id].find(tp);
            if (tfit == docTf[id].end()) continue;
...
const string* Indexer::lookupTermPtr(const string &term) const {
    ...
    if (it != vocabMap.end()) return it->second;
    return nullptr;
}
...
unordered_set<int> Indexer::getDocIDsForTerm(const string &term) const {
    ...
    if (it == postingsSharded[sh].end()) return out;
    for (auto &p : it->second) out.insert(p.docID);
    return out;
}
...
vector<int> Indexer::getDocIDsSorted(const string &term) const {
    ...
    lock_guard<mutex> lk(*shardMutexes[sh]);
    auto it = postingsSharded[sh].find(tp);
    if (it == postingsSharded[sh].end()) return {};
    vector<int> out;
    out.reserve(it->second.size());
    for (auto &p : it->second) out.push_back(p.docID);
...
const vector<Posting>* Indexer::getPostings(const string &term) const {
    ...
    lock_guard<mutex> lk(*shardMutexes[sh]);
    auto it = postingsSharded[sh].find(tp);
    if (it == postingsSharded[sh].end()) return nullptr;
    return &it->second;
}
...
bool Indexer::docHasPhrase(int docID, const vector<string> &terms) const {
    ...
    for (auto &t : terms) {
        ...
        lock_guard<mutex> lk(*shardMutexes[sh]);
        auto it = postingsSharded[sh].find(tp);
        if (it == postingsSharded[sh].end()) return false;
        const vector<Posting> &plist = it->second;
        const vector<int> *found = nullptr;
        for (auto &post : plist) if (post.docID == docID) { found = &post.positions; break; }
    ...
    for (int p : *posLists[0]) {
        ...
        for (size_t i = 1; i < posLists.size(); ++i) {
            if (find(posLists[i]->begin(), posLists[i]->end(), p + (int)i) == posLists[i]->end()) { ok = false; break; }
        }
        if (ok) return true;
    }
    return false;
...
int Indexer::termFreqInDoc(const string &term, int docID) const {
    ...
    lock_guard<mutex> lk(*shardMutexes[sh]);
    auto it = postingsSharded[sh].find(tp);
    if (it == postingsSharded[sh].end()) return 0;
    for (auto &p : it->second) if (p.docID == docID) return (int)p.positions.size();
    return 0;
}
...
void Indexer::removeDocument(int docID) {
    if (docID < 0 || docID >= (int)docs.size()) return;
    for (int sh = 0; sh < shardCount; ++sh) {
        lock_guard<mutex> lk(*shardMutexes[sh]);
        for (auto &entry : postingsSharded[sh]) {
            auto &plist = entry.second;
            plist.erase(remove_if(plist.begin(), plist.end(), [&](const Posting &p){ return p.docID == docID; }), plist.end());
        }
    }
    docTf[docID].clear();
    ...
    { lock_guard<mutex> lk(contentCacheMu); contentCache.erase(docID); }
    buildVocabStructures();
}
...
string Indexer::loadContent(int docID) const {
    ...
    // Check cache first
    {
        ...
        if (it != contentCache.end()) return it->second;
    }
...
string Indexer::makeSnippet(int docID, const vector<string> &qterms, size_t window) const {
    ...
    sort(idx.begin(), idx.end(), [&](size_t a, size_t b){ return hits[a] < hits[b]; });
    ...
    for (size_t r = 0; r < idx.size(); ++r) {
        ...
        for (auto &q : qterms) {
            string ql = q;
            transform(ql.begin(), ql.end(), ql.begin(), ::tolower);
            if (lower.substr(pos, ql.size()) == ql) { termKey = ql; break; }
        }
        if (termKey.empty()) continue;

        if (inWindow[termKey]++ == 0) distinctInWindow++;
        ...
        while (left < r && pos - hits[idx[left]] > window) {
            ...
            for (auto &q : qterms) {
                string ql = q;
                transform(ql.begin(), ql.end(), ql.begin(), ::tolower);
                if (lower.substr(lpos, ql.size()) == ql) { lkey = ql; break; }
            }
            if (!lkey.empty() && --inWindow[lkey] == 0) distinctInWindow--;
            ++left;
        }
    ...
    if (s != string::npos) snip = snip.substr(s, e - s + 1);

    if (start > 0)              snip = "..." + snip;
    if (end < content.size())   snip += "...";
...
vector<pair<string,int>> Indexer::getTopTerms(int k) const {
    ...
    if ((int)terms.size() > k) terms.resize(k);
    return terms;
}
