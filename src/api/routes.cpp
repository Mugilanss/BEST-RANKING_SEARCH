...
static bool checkRate(const httplib::Request &req,
                      ...
                      EngineContext          &ctx)
{
    if (!ctx.rateLimiter.allow(req.remote_addr)) {
        addCORS(res);
        res.status = 429;
...
static string sanitizeInput(const string &s, size_t maxLen = 512) {
    ...
    for (char c : s) {
        ...
        if ((unsigned char)c >= 0x20) out += c; // drop control chars
    }
    return out;
}
                        ...
                        uint64_t &latencyMs)
{
    ...
    for (size_t i = 0; i < results.size(); ++i) {
        ...
        string snippet = ctx.indexer.makeSnippet(r.id, qtokens, 80);
...
void register_routes(httplib::Server &srv, EngineContext &ctx) {
    ...
    // ------------------------------------------------------------------
    // POST /index/update  — ADMIN (6.5 token auth)
    // ------------------------------------------------------------------
    srv.Post("/index/update", [&ctx](const httplib::Request &req, httplib::Response &res) {
        ...
        lock_guard<mutex> lk(ctx.engineMu);
        auto t0 = chrono::steady_clock::now();
        ctx.indexer.incrementalUpdateWithWAL(ctx.cfg.docsFolder, ctx.wal, true);
        if (!ctx.cfg.indexFile.empty()) ctx.indexer.saveIndex(ctx.cfg.indexFile);
        ctx.cache->clear();
        auto ms = chrono::duration_cast<chrono::milliseconds>(
            chrono::steady_clock::now() - t0).count();
    ...
    // ------------------------------------------------------------------
    // POST /crawl?url=<seed>&depth=<n>&pages=<n>  — ADMIN (6.4 + 6.5)
    // Body: optional JSON with url/depth/pages
    // ------------------------------------------------------------------
    srv.Post("/crawl", [&ctx](const httplib::Request &req, httplib::Response &res) {
        ...
        // Run crawl in background thread, respond immediately
        ...
        thread([&ctx, seedUrl, depth, pages, outputDir]() {
            ...
            int crawled = crawler.crawl(seedUrl, [&ctx](const string &filepath) {
                // Incrementally index each page as it's saved
                lock_guard<mutex> lk(ctx.engineMu);
